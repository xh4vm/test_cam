...
async def test_user_login_success(
    generate_users, generate_user_sessions, sqlite_get_request, make_request
):
    user = generate_users[0]

    login_data = await sqlite_get_request(
        model=FakeUserSession, table="user_session", user_id=user.id
    )
...
async def test_user_login_missing(
    generate_users, generate_user_sessions, sqlite_get_request, make_request
):
    user = generate_users[0]
