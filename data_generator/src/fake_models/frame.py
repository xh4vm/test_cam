...
class FakeFrame(BaseModel):
    ...
    TimeSection: str = Field(default_factory=lambda: datetime.utcnow().strftime('%d-%m-%Y:%H-%M-%S'))
    ChannelNo: int = Field(default_factory=lambda:randint(1,2))
    ConfigNo: int = Field(default_factory=lambda:randint(0,1))
    contributors: list[int] = Field(default_factory=lambda:[randint(1,100) for i in range(randint(1,10))])
